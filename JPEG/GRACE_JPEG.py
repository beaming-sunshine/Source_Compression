...
def get_ds():
    img_dct = torch.randn(16,16)
    img_s_var = Variable(img_dct,requires_grad=True)
    img_idct = idct(img_s_var) # 使用idct获得img的原始图像 
    img_idct.sum().backward()
    ds = img_s_var.grad
    return ds

def get_dx(datasetdir,model):
