...
class KJPEG:
    def __init__(self, WR, WG, WB, TY, TU, TV):
        # 初始化DCT变换的A矩阵，https://blog.csdn.net/ahafg/article/details/48808443
        self.__dctA = np.zeros(shape=(16, 16))
        for i in range(16):
            c = 0
            if i == 0:
                c = np.sqrt(1 / 16)
                ...
                c = np.sqrt(2 / 16)
            for j in range(16):
                self.__dctA[i, j] = c * np.cos(np.pi * i * (2 * j + 1) / (2 * 16))
        # 参数
        self.__WR = WR
        self.__WG = WG
        self.__WB = WB
    ...
    def __Encode(self, matrix, tag):
        ...
        shape = (height // 16, width // 16, 16, 16)
        strides = matrix.itemsize * np.array([width * 16, 16, width, 1])
        blocks = np.lib.stride_tricks.as_strided(matrix, shape=shape, strides=strides)
        res = []
        for i in range(height // 16):
            for j in range(width // 16):
                res.append(self.__Quantize(self.__Dct(blocks[i, j]).reshape(256), tag))
        return res
    ...
    def __Decode(self, blocks, tag):
        ...
        for b in blocks:
            b = np.array(b)
            tlist.append(self.__IDct(self.__IQuantize(b, tag).reshape(16 ,16)))
        height_fill, width_fill = self.height, self.width
        if height_fill % 32 != 0:
            height_fill += 32 - height_fill % 32
        if width_fill % 32 != 0:
            width_fill += 32 - width_fill % 32
        rlist = []
        for hi in range(height_fill // 16):
            start = hi * width_fill // 16
            rlist.append(np.hstack(tuple(tlist[start: start + (width_fill // 16)])))
    ...
    def Decompress(self, y_dcode, u_dcode, v_dcode):
